...
namespace InventoryManagementSystem
{
    public partial class ProductForm : Form
    {
        ...
        public ProductForm()
        {
            ...
            // subscribe to resequence notifications so UI updates when IDs change
            ...
            try
            {
                if (table == "tbProduct")
                {
                    ...
                    if (this.IsHandleCreated)
                        this.BeginInvoke(new Action(LoadProduct));
                }
            }
            catch { }
        }
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this product?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    // Resequence product IDs and update child references (tbOrder.pid)
                    try
                    {
                        DatabaseResequencer.ResequenceTable(con.ConnectionString, "tbProduct", "pid", new[] { "tbOrder.pid" });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Resequencing failed: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            LoadProduct();
