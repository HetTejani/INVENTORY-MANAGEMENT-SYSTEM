            ...
            try
            {
                if (table == "tbCustomer")
                {
                    if (this.IsHandleCreated)
                        this.BeginInvoke(new Action(LoadCustomer));
                }
            }
            catch { }
        }
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this customer?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    // Resequence customer IDs and update child references (tbOrder.cid)
                    try
                    {
                        DatabaseResequencer.ResequenceTable(con.ConnectionString, "tbCustomer", "cid", new[] { "tbOrder.cid" });
                    }
                    catch (Exception ex)
                    {
