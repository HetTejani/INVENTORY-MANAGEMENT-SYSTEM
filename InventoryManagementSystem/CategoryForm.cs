            ...
            try
            {
                if (table == "tbCategory")
                {
                    if (this.IsHandleCreated)
                        this.BeginInvoke(new Action(LoadCategory));
                }
            }
            catch { }
        }
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this category?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    // Resequence category IDs (no child references known)
                    try
                    {
                        DatabaseResequencer.ResequenceTable(con.ConnectionString, "tbCategory", "catid", null);
                    }
                    catch (Exception ex)
                    {
