...
namespace InventoryManagementSystem
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\lenovo\Documents\dbIMS.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        private void OnDataChanged(string table)
        {
            try
            {
                if (table == "tbOrder" || table == "tbProduct" || table == "tbCustomer")
                {
                    if (this.IsHandleCreated)
                        this.BeginInvoke(new Action(LoadOrder));
                }
            }
            catch { }
        }
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this order?", "Delete Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    // Resequence order IDs and update any child references (none in current schema)
                    try
                    {
                        DatabaseResequencer.ResequenceTable(con.ConnectionString, "tbOrder", "orderid", null);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Reseching failed: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }

                    // Notify other forms
                    DataChangeNotifier.Notify("tbOrder");
                    DataChangeNotifier.Notify("tbProduct");
                }
            }
            LoadOrder();
        }
